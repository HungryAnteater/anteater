...
class AntStringTable
{
	vector<cstr> strings;
	unordered_map<cstr, int> toId;

public:

	int operator[](cstr str)
	{
		auto i = toId.find(str);
		if (i != toId.end())
			return i->second;

		int id = (int)strings.size();
		strings.push_back(str);
		toId.emplace(str, id);
		return id;
	}

	cstr operator[](int id) const
	{
		return strings.at(id);
	}
};

// Is just an int but represents a string and wraps the lookup functionality
struct AntString
{
	const int id;

	AntString(cstr s): id(GetID(s)) {}
	AntString(const string& s): AntString(s.c_str()) {}

	cstr AsString() const { return GetString(id); }
	...
	{
		static AntStringTable* t = new AntStringTable();
		return *t;
	}
};
...
inline bool IsNumber(AntType t) { return t==ANT_INT || t==ANT_FLOAT; }

template<class T> AntType antType  = ANT_INVALID;
template<> AntType antType<int>    = ANT_INT;
template<> AntType antType<float>  = ANT_FLOAT;
template<> AntType antType<string> = ANT_STRING;
template<> AntType antType<cstr>   = ANT_STRING;

template <class T> AntType GetAntType() { return antType<T>; }

inline AntType CommmonType(AntType a, AntType b)
{
	if (a == b) return a;
	if (a == ANT_STRING || b == ANT_STRING) return ANT_STRING;
	if (IsNumber(a) && IsNumber(b))
	{
		if (a == ANT_FLOAT || b == ANT_FLOAT) return ANT_FLOAT;
	}
	return ANT_INVALID;
}
...
public:
	AntType type;
	variant<nullptr_t, int, float, vector<AntValue>> data;
